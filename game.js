@@ -1 +1,3985 @@
// SEE ARTIFACTS - PLACEHOLDER AVOID
// SEE ARTIFACTS - PLACEHOLDER AVOID// @ts-nocheck
"use strict";
...
var COSMETICS = [
	...
	{ id: "face_smile", slot: "face", nameKey: "face_smile", cost: 25, color: "#ffb703" }
];
	...
	static load() {
		try {
			...
			return {
				...
				ownedCosmetics: Array.isArray(d.ownedCosmetics) ? d.ownedCosmetics : [],
				options: {
...
function drawCosmeticItem(ctx, id, scale = 1) {
	...
	if (id === "hat_cap") {
		...
		ctx.beginPath();
		ctx.moveTo(-14, -14); ctx.lineTo(-10, -28); ctx.lineTo(-5, -16);
		ctx.lineTo(0, -30); ctx.lineTo(5, -16); ctx.lineTo(10, -28); ctx.lineTo(14, -14);
		ctx.closePath(); ctx.fill();
		ctx.fillStyle = "#e63946";
		[[-10,-26],[0,-28],[10,-26]].forEach(([x,y]) => { ctx.beginPath(); ctx.arc(x,y,2.5,0,Math.PI*2); ctx.fill(); });
	} else if (id === "hat_wizard") {
		ctx.fillStyle = "#7b2cbf";
		ctx.beginPath();
		ctx.moveTo(-12, -12); ctx.lineTo(0, -38); ctx.lineTo(12, -12); ctx.closePath(); ctx.fill();
		ctx.fillStyle = "#ffd60a";
		ctx.beginPath(); ctx.arc(0, -30, 3, 0, Math.PI*2); ctx.fill();
	} else if (id === "hat_chef") {
		ctx.fillStyle = "#f8f9fa";
		ctx.beginPath(); ctx.ellipse(0, -22, 14, 10, 0, 0, Math.PI*2); ctx.fill();
		ctx.fillRect(-10, -16, 20, 8);
	} else if (id === "glasses_sun") {
		ctx.fillStyle = "#212529";
		ctx.fillRect(-14, -4, 10, 8); ctx.fillRect(4, -4, 10, 8);
		...
		ctx.beginPath();
		ctx.ellipse(-6, 10, 8, 4, -.2, 0, Math.PI*2);
		ctx.ellipse(6, 10, 8, 4, .2, 0, Math.PI*2);
		ctx.fill();
	} else if (id === "suit_cape") {
		ctx.fillStyle = "rgba(193,18,31,0.7)";
		ctx.beginPath();
		ctx.moveTo(-12, 8); ctx.quadraticCurveTo(-22, 30, -8, 40);
		ctx.lineTo(8, 40); ctx.quadraticCurveTo(22, 30, 12, 8); ctx.fill();
...
function drawAura(ctx, id, r) {
	...
	ctx.save();
	if (id === "aura_green") {
		ctx.strokeStyle = `rgba(76,159,56,${0.35 + Math.sin(t*3)*0.15})`;
		ctx.lineWidth = 3;
		ctx.beginPath(); ctx.arc(0, 0, r + 6 + Math.sin(t*2)*2, 0, Math.PI*2); ctx.stroke();
	} else if (id === "aura_gold") {
		ctx.strokeStyle = `rgba(255,214,10,${0.4 + Math.sin(t*4)*0.2})`;
		ctx.lineWidth = 3;
		...
		for (let i = 0; i < 6; i++) {
			...
			ctx.beginPath(); ctx.arc(Math.cos(a)*(r+12), Math.sin(a)*(r+12), 2.5, 0, Math.PI*2); ctx.fill();
		}
	} else if (id === "aura_fire") {
		for (let i = 0; i < 8; i++) {
			const a = t * 3 + i * Math.PI / 4;
			const rr = r + 4 + Math.sin(t*5 + i)*4;
			ctx.fillStyle = `rgba(232,93,4,${0.35 + Math.sin(t*4+i)*0.2})`;
			ctx.beginPath(); ctx.arc(Math.cos(a)*rr*0.3, -rr*0.6 + Math.sin(a)*4, 4, 0, Math.PI*2); ctx.fill();
		}
	}
	ctx.restore();
}
function drawMascot(ctx, x, y, cosmetics, scale = 1) {
	...
	ctx.beginPath(); ctx.ellipse(0, -18, 8, 4, 0, 0, Math.PI*2); ctx.fill();
...
class Store {
	...
	buildButtons() {
		...
		const tabs = ["upgrades", "colors", "cosmetics"];
		const tabLabels = [g.t("upgrades"), g.t("colors"), g.t("cosmetics")];
...
class Game {
	...
	buildOptions() {
		...
		this.menuButtons = [
			...
			new Button(w / 2 - bw / 2, y + gap * 6, bw, 44, this.t("back"), {
				color: "#4c9f38",
				onClick: () => {
					this.state = "menu";
					this.buildMenu();
	...
	if (this.state === "credits") {
		...
		if (this.level > prevLevel) {
			this.sound.levelUp();
			this.spawnFloat(this.w / 2, this.h * 0.3, `${this.t("level")} ${this.level}!`, "#ffd60a");
	...
	updateVS(dt) {
		...
		if (this.spawnTimer <= 0) {
			this.spawnVSFood(1);
			this.spawnVSFood(2);
			this.spawnTimer = Math.max(0.35, 0.85 - this.timeAlive * 0.008);
		}
		for (let i = this.foods.length - 1; i >= 0; i--) {
			const f = this.foods[i];
	...
	smashVSFood(f) {
		...
		if (def.kind === "junk") {
			...
			const pts = (def.score || 10);
			if (f.side === 1) this.p1Score += pts;
	...
	drawVS(ctx, w, h) {
		...
		roundRect(ctx, 10, 10, w * 0.42, 56, 12);
		ctx.fill();
		roundRect(ctx, w * 0.58 - 10, 10, w * 0.42, 56, 12);
		ctx.fill();
		ctx.fillStyle = "#4c9f38";
		...
		ctx.fillText("❤".repeat(Math.max(0, this.p1Lives || 0)), 22, 50);
		ctx.textAlign = "right";
		ctx.fillText("❤".repeat(Math.max(0, this.p2Lives || 0)), w - 22, 50);
		ctx.restore();
		...
		if (this.state === "vs_intro") {
			...
			ctx.textAlign = "center";
			ctx.fillText(this.t("vsIntro"), w / 2, h * 0.58);
		}
		if (this.state === "vs_result") {
			ctx.fillStyle = "rgba(20,40,29,0.72)";
			ctx.fillRect(0, 0, w, h);
			const winner = this.vsWinner;
			ctx.fillStyle = winner === 1 ? "#4c9f38" : "#e85d04";
			ctx.font = `900 36px ${FONT}`;
			ctx.textAlign = "center";
			ctx.fillText(this.t("vsWin"), w / 2, h * 0.38);
			ctx.fillStyle = "#f4f1e8";
			ctx.font = `800 22px ${FONT}`;
			ctx.fillText(winner === 1 ? this.t("p1") : this.t("p2"), w / 2, h * 0.48);
			drawMascot(ctx, w / 2, h * 0.28, this.data.cosmetics, 1.3);
	...
	draw() {
		...
		if ([
			...
			if (this.state !== "pause") {
				...
				if (this.state === "menu") drawMascot(ctx, w / 2, Math.min(h * .16, 110) + 78, this.data.cosmetics, 0.85);
			}
